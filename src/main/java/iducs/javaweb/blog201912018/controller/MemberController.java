...
import iducs.javaweb.blog201912018.repository.MemberDAO;
import iducs.javaweb.blog201912018.repository.MemberDAOImpl;
...
public class MemberController extends HttpServlet {
    ...
    protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(action.equals("list.do")) { // member listing

            List<Member> memberList = new ArrayList<Member>();
            if((memberList = memberDAOImpl.readList()) != null) {
                request.setAttribute("members", memberList);
                request.getRequestDispatcher("../members/list.jsp").forward(request, response); //정상
            }
            else
            ...
            System.out.println("일단 컨트롤러로는 들어와서 한다");
            if((ret = memberDAOImpl.create(m)) > 0) {
                System.out.println("성공하면 일로 들어와짐");
