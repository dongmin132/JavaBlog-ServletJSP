...
public class MemberDAOImpl extends OracleDAOImpl implements MemberDAO {
    ...
    public int create(Member m) {
        ...
        String sql = "insert into member1 values(seq_member1.nextval, '" +
                 ...
                 m.getAddress() + "')";
         */
        String sql = "insert into m201912018 values(seq_member.nextval,?,?,?,?,?)";
    ...
    public Member read(Member m) {
        ...
        String sql = "select * from m201912018 where email=?";
        try {
            conn = getConnection(); // DB 연결 객체 생성
            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, m.getEmail());
    ...
    public List<Member> readList() {
        ...
        Member retMember = null;
        String sql = "select * from m201912018";
        try {
            conn = getConnection(); // DB 연결 객체 생성
            stmt = conn.createStatement();
    ...
    public int update(Member m) {
        int ret = 0;
        String sql = "update m201912018 set name=?, phone=?, address=? where email=?";
        // placeholder : 자리 대체 기호
        try {
            conn = getConnection(); // DB 연결 객체 생성
            pstmt = conn.prepareStatement(sql);
    ...
    public int delete(Member m) {
        int ret = 0;
        String sql = "delete from m201912018 where email=? and pw=?";
        // placeholder : 자리 대체 기호
        try {
            conn = getConnection(); // DB 연결 객체 생성
            pstmt = conn.prepareStatement(sql);
    ...
    public Member login(Member m) {
        Member retMember = null;
        String sql = "select * from m201912018 where email=? and pw=?";
        try {
            conn = getConnection(); // DB 연결 객체 생성
            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, m.getEmail());
